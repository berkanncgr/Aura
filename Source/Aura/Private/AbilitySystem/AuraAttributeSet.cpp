...
void UAuraAttributeSet::PostAttributeChange(const FGameplayAttribute& Attribute, float OldValue, float NewValue)
{
	...
	{
		//SetMaxHealth(GetMaxHealth()); // This Looks ridiculous.
		bTopOffHealth = false;
	}

	if (Attribute == GetMaxManaAttribute() && bTopOffMana)
	{
		//SetMaxMana(GetMaxMana()); // This Looks ridiculous.
		bTopOffMana = false;
	}
}
...
{
	// There is no debuff:
	if (!UAuraAbilitySystemLibrary::IsSuccessfulDebuff(Props.EffectContextHandle)) return;
	Debuff(Props); //There is debuff:
}
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (bFatal)
	{
		...
		if (CombatInterface) CombatInterface->Die(UAuraAbilitySystemLibrary::GetDeathImpulse(Props.EffectContextHandle));
		SendXPEvent(Props);
	}
	else
	{
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectProperties& Props)
{
	...
	if (!Props.SourceCharacter->Implements<UPlayerInterface>() || !Props.SourceCharacter->Implements<UCombatInterface>()) return;
